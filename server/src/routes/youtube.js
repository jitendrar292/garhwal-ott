...
router.get('/search', async (req, res) => {
  try {
    const { q, pageToken, maxResults, order, videoCategoryId } = req.query;
    ...
    // Only allow numeric videoCategoryId values (YouTube category IDs)
    const safeCategoryId = /^\d+$/.test(videoCategoryId || '') ? videoCategoryId : '';
    const data = await searchVideos(
      q.trim(),
      pageToken || '',
      clamped,
      { order: safeOrder, videoCategoryId: safeCategoryId }
    );
    res.json(data);
  } catch (err) {
