...
function isLikelyValidJob(job) {
  if (!job || !job.title || !job.department || !job.lastDate) return false;
...
function sanitizeJob(job) {
  return {
    ...
    lastDate: String(job.lastDate || '').trim(),
    salary: String(job.salary || '').trim(),
    eligibility: String(job.eligibility || '').trim(),
    category: VALID_CATEGORIES.has(job.category) ? job.category : 'state',
    link: String(job.link || '').trim(),
    emoji: String(job.emoji || '📋').trim() || '📋',
    featured: job.featured === true || (parseInt(job.vacancies, 10) || 0) >= 100,
  };
}
...
router.post('/publish-jobs', async (req, res) => {
  ...
  try {
    ...
    for (const job of selected) {
      if (!job.title || !job.department || !job.lastDate) {
        skipped++;
        continue;
      }
    ...
    console.log(`[jobsAgent] Published ${published} jobs, skipped ${skipped}`);
    res.json({ message: `Published ${published} jobs`, published, skipped });
  } catch (err) {
