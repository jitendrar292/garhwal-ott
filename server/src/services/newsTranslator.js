...
async function getSavedNewsExcerpts() {
  ...
  try {
    ...
    savedNewsCache = articles
      ...
      .slice(0, 50) // keep recent 50 for matching
      .map((a) => ({
        title: a.title,
        body: a.body.slice(0, 300), // first 300 chars as excerpt
        category: a.category || '',
      }));
    savedNewsCacheTime = Date.now();
  } catch {
    // If loadNews fails (e.g. on cold start), return empty
    savedNewsCache = [];
  }
  return savedNewsCache;
}
 ...
 * Uses keyword overlap to pick the most relevant past translations.
 */
function selectRelevantSavedNews(articleText, savedArticles, maxExamples = 3) {
  ...
  const scored = savedArticles.map((saved) => {
    const savedWords = `${saved.title} ${saved.body}`.toLowerCase().split(/\s+/);
    const hits = savedWords.filter((w) => w.length > 3 && wordSet.has(w)).length;
    return { saved, score: hits };
  });
...
async function buildRAGContext(article) {
  ...
  const pastNews = selectRelevantSavedNews(fullText, savedArticles);
  ...
  if (pastNews.length > 0) {
    ragBlock += '\n\n## पहले प्रकाशित गढ़वळि समाचार (Match this writing style exactly):\n';
