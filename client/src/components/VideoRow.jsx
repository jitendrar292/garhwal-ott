  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h2 className="section-header border-l-[3px] border-primary-500/70 pl-3">
            <span>{title}</span>
            {categoryLink && (
              <Link
                to={categoryLink}
          ...
          {subtitle && (
            <p className="text-caption text-white/40 flex items-center gap-1 mt-1 pl-3">
      ...
      {/* Scrollable row */}
      {loading ? (
        <div className="scroll-row" role="status" aria-label={`Loading ${title}`}>
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className={`${i === 0 ? 'w-[240px] sm:w-[280px] md:w-[300px]' : 'w-[200px] sm:w-[240px] md:w-[260px]'} rounded-2xl overflow-hidden bg-surface-2 border border-white/5`}>
        ...
        <div className="empty-state">
          ...
          <p className="empty-state-title">कोई वीडियो नहीं मिला</p>
          <p className="empty-state-body">This section has no videos right now. Check back soon!</p>
        </div>
      ) : (
        <div ref={scrollRef} className="scroll-row" role="list">
          {videos.map((video, i) => (
            <div
              key={video.id}
              role="listitem"
